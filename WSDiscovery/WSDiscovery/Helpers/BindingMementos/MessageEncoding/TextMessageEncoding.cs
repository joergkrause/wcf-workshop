...
namespace Masieri.ServiceModel.WSDiscovery.Helpers.BindingMementos.MessageEncoding
{
  ...
  class TextMessageEncoding
  {
    ...
    /// <returns></returns>
    public static TextMessageEncodingBindingElement ReadXml(XmlReader reader)
    {
      TextMessageEncodingBindingElement enc = new TextMessageEncodingBindingElement();
      if (reader.LocalName != "TextMessageEncodingBindingElement")
        reader.ReadStartElement("TextMessageEncodingBindingElement");
      do
      {
        if (!reader.IsStartElement() && reader.LocalName == "TextMessageEncodingBindingElement")
          break;
        else
        {
    ...
    /// <returns></returns>
    public static string WriteXml(TextMessageEncodingBindingElement enc)
    {
      StringBuilder sb = new StringBuilder();
      TextMessageEncodingBindingElement el = new TextMessageEncodingBindingElement();
      using (XmlTextWriter w = new XmlTextWriter(new StringWriter(sb)))
      {
        //Comprimere
        w.WriteStartElement("TextMessageEncodingBindingElement");
