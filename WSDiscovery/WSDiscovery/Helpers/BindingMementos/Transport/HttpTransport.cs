...
namespace Masieri.ServiceModel.WSDiscovery.Helpers.BindingMementos.Transport
{
  ...
  class HttpTransport
  {
    ...
    /// <returns></returns>
    public static HttpTransportBindingElement ReadXml(XmlReader reader)
    {
      HttpTransportBindingElement tran = new HttpTransportBindingElement();
      if (reader.LocalName != "HttpTransportBindingElement")
        reader.ReadStartElement("HttpTransportBindingElement");
      do
      {
        if ((!reader.IsStartElement()) || (reader.IsEmptyElement) && reader.LocalName == "HttpTransportBindingElement")
          break;
        else
        {
    ...
    /// <returns></returns>
    public static string WriteXml(HttpTransportBindingElement tran)
    {
